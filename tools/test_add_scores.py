...
from .add_scores import calculate_totals, calulate_week_diff
...
class TestTools(unittest.TestCase):
    ...
    def test_calulate_week_diff_ok(self):
        next_week = {'a': 3, 'b': 10, 'z': 10, 'x': 5, 'c': 3, 'y': 1, 'w': 10}
        expected = {'b': 3, 'x': 3, 'w': 10}
        diff = calulate_week_diff(self.test_week_data, next_week, 4)
        assert expected == diff

    def test_calculate_week_diff_missing_riders(self):
        next_week = {'a': 3, 'b': 7, 'z': 10, 'x': 2, 'c': 3}
        self.assertRaises(KeyError, lambda: calulate_week_diff(self.test_week_data, next_week, 4))
    ...
    def test_calculate_week_diff_wrong_week(self):
        next_week = self.totals
        self.assertRaises(ValueError, lambda: calulate_week_diff(self.test_week_data, next_week, 5))
